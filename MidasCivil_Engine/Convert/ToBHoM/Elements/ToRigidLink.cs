...
namespace BH.Engine.External.MidasCivil
{
    public static partial class Convert
    {
        public static RigidLink ToRigidLink(string rigidLink, Dictionary<string,Node> nodes, int count)
        {
            ...
            List<int> assignments = Engine.External.MidasCivil.Query.Assignments(slaves);

            bool x = Engine.External.MidasCivil.Convert.FromFixity(fixity.Substring(0, 1));
            bool y = Engine.External.MidasCivil.Convert.FromFixity(fixity.Substring(1, 1));
            bool z = Engine.External.MidasCivil.Convert.FromFixity(fixity.Substring(2, 1));
            bool xx = Engine.External.MidasCivil.Convert.FromFixity(fixity.Substring(3, 1));
            bool yy = Engine.External.MidasCivil.Convert.FromFixity(fixity.Substring(4, 1));
            bool zz = Engine.External.MidasCivil.Convert.FromFixity(fixity.Substring(5, 1));
